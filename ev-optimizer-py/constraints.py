...
def add_distance_dimension(routing: pywrapcp.RoutingModel, manager: pywrapcp.RoutingIndexManager, distance_matrix: List[List[int]], max_ranges: List[float]) -> pywrapcp.RoutingDimension:
    """
...
def add_charging_constraints(distance_dimension: pywrapcp.RoutingDimension, manager: pywrapcp.RoutingIndexManager, charging_stations: List[Dict[str, Any]], max_ranges: List[float]):
    """
...
def add_time_windows(routing: pywrapcp.RoutingModel, manager: pywrapcp.RoutingIndexManager, distance_matrix: List[List[int]], deliveries: List[Dict[str, Any]], depots, time_limit: int = 10000):
    """
...
def restrict_unavailable_vehicles(routing: pywrapcp.RoutingModel, drivers: List[Dict[str, Any]], num_vehicles: int):
    """
    Restricts assignment of routes to unavailable vehicles.
    """
    available_vehicles = [i for i, drv in enumerate(drivers) if drv.get("available", True)]
