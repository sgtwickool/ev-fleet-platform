...
from constraints import add_distance_dimension, add_time_windows, restrict_unavailable_vehicles
...
class EVFleetOptimizer:
    ...
    def optimize_vehicle_routes(
        ...
        distance_dimension = add_distance_dimension(routing, manager, distance_matrix, max_ranges)
        if charging_stations:
            from constraints import add_charging_constraints
            add_charging_constraints(distance_dimension, manager, charging_stations, max_ranges)
        ...
        if solution:
            ...
            charging_locations = set(cs["location"] for cs in charging_stations) if charging_stations else set()
            routes = extract_routes(
                solution, routing, manager, num_vehicles, max_ranges, charging_locations, distance_matrix
            )
