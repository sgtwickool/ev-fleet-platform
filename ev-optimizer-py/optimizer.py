...
def optimize_vehicle_routes(
    ...
    if any('time_window' in d for d in deliveries):
        ...
        def time_callback(from_index, to_index):
            from_node = manager.IndexToNode(from_index)
            to_node = manager.IndexToNode(to_index)
            return distance_matrix[from_node][to_node]  # or use travel time if available
    ...
    if solution:
        for vehicle_id in range(num_vehicles):
            ...
            while not routing.IsEnd(index):
                ...
                distance = distance_matrix[node][manager.IndexToNode(next_index)] if not routing.IsEnd(next_index) else 0
                soc -= distance
                if node in charging_locations:
                    soc = max_ranges[vehicle_id]  # recharge
                index = next_index
            route.append({'location': manager.IndexToNode(index), 'soc': soc, 'is_charging_station': False})
            routes.append(route)
        return {
            "routes": routes,
